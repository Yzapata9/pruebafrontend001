...
export default function ToDoList() {
  ...
  //e es un evento, como tipear algo o una checkbox
  function handleToDoChange(e, id) {
    //target determina el elemento que es e (typing o checkbox)
    const target = e.target
    // si es un checkbox ejecuta la func .checked, sino(:) .value
  ...
  async function addToDo(name) {
    ...
    if (res.ok) {
      const json = await res.json();
      //el ,json final lo que hace es agegar un item a la copia de ...todos, alfinal de la cadena
  ...
  async function handleDeleteToDo(id) {
    ...
    if (res.ok) {
      //busca en el array todo, la variable id (todo.id) el id que viene de la funcion
      const idx = todos.findIndex((todo) => todo.id === id)
      //crea una copia de todos para actualizarla
      const copy = [...todos]
      //splice borra "1" item de la copia del array todos, con el indice idx, que es el que encontramos con findIndex
      copy.splice(idx, 1)
      setTodos(copy)
    }
  }
  ...
  //funcion para responder a acciones del teclado
  function handleKeyDown(e) {
    //si apretan el enter
    if (e.key === 'Enter') {
      //si maininput no esta vacia
      if (mainInput.length > 0) {
        //agrega el item maininput
        addToDo(mainInput)
        //limpia el textfiel maininput
        setMainInput('')
      }
    }
  }
